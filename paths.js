...
const SC_SPRITES_DIR = path.join(PROJECT_ROOT, 'vendor-sc', 'img', 'Charas');

// Legacy PokeAPI paths (kept for backward compat if sprites exist)
...
function spriteCandidates(kind, fileName) {
  if (kind === 'static') {
    return [path.join(GEN5_STATIC_DIR, fileName)];
  }
  if (kind === 'animated') {
    return [path.join(GEN5_ANIMATED_DIR, fileName)];
  }
  if (kind === 'icon') {
    return [path.join(GEN5_ICON_ANIMATED_DIR, fileName), path.join(GEN5_ICON_DIR, fileName)];
  }
  if (kind === 'icon-static') {
    return [path.join(GEN5_ICON_DIR, fileName)];
  }
  return [];
}

function spriteSheetPath(unitName) {
  return path.join(SC_SPRITES_DIR, unitName + '.png');
}
